...
namespace Business.Services
{
    public class TareaService : ITareaService
    {
        ...
        public void CargarTarea(TareaDTO tareaDTO)
        {
            ...
            if (tareaDTO.FechaInicio.Date < DateTime.Today)
                throw new TareaException("La fecha de inicio no puede ser anterior a hoy");
            ...
            tarea.FechaVencimiento = tareaDTO.FechaInicio.Date.AddDays(tareaDTO.EstimacionDias);

            tarea.Vencida = tarea.FechaVencimiento < DateTime.Today;
        ...
        public List<TareaResponseDTO> GetTareasByProyectoId(Guid proyectoId)
        {
            List<TareaResponseDTO> tareas =
                _mapper.Map<List<TareaResponseDTO>>(_tareaRepository.GetAllByIdProyecto(proyectoId));

            foreach (TareaResponseDTO tarea in tareas)
            {
                tarea.Miembros = _mapper.Map<List<UsuarioDTO>>(
                    _tareaRepository.GetMiembrosByTareaId(tarea.TareaId));
            }
