$(document).ready(function () {
    ...
    $('#selectResponsable').on('change blur', function () {
        let val = $(this).val();
        $(this).nextAll('.text-danger.small').first().empty();
        $(this).removeClass('is-invalid');
        if (val === "" || val === null) {
            ...
            let validationSpan = $(this).nextAll('.text-danger.small').first();
            if (validationSpan.length) {
                validationSpan.text('Debes seleccionar un responsable para el proyecto.');
            } else {
            }
        }
